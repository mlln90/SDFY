...
var addbirthInit = function(_birthYear, _birthMonth, _birthDay, defaultBirthYear, defaultBirthMonth, defaultDay){

    var birthyear = document.getElementById(_birthYear);  
    var birthmonth = document.getElementById(_birthMonth);  
    var birthday = document.getElementById(_birthDay); 

    var y = new Date().getFullYear();   // 今年年份
    var yafter  = Number(y);           
    for(var i=yafter; i>1900; i--){
        var option = document.createElement("option");
    ...
    for(var j=1; j<13; j++){
        ...
        var valueM = ''+j; 
        optionM.setAttribute("value",j);
        if(j<10){ valueM = '0' + j};
        optionM.innerHTML = valueM;
        birthmonth.appendChild( optionM );
    }
    for(var k=1; k<=31; k++){
        var optionD = document.createElement("option");
        var valueD = ''+k;
        optionD.setAttribute("value",k);
        if(k<10){ valueD = '0' + k}; 
        optionD.innerHTML = valueD;
        birthday.appendChild( optionD );
    }
}
