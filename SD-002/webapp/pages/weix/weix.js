	...
	switch (_type) {
		...
		case 1: 
			if (WEIXSTATE) {
				...
				$.layerPop({
					msg: '请确认微信功能开启？ </br> 微信扫码相关功能将开启使用',
					time: 6,
					yes: function() {
						BankUpDown('true');		
					}
		...
		case 2:
			if (WEIXSTATE) {
				$.layerPop({
					msg: '请确认微信扫码功能关闭？ </br> 微信扫码相关功能将暂停使用',
					time: 6,
					yes: function() {
						BankUpDown('false');
					}
...
function QueryBankList() {
	...
	// console.log(param);
	// AJAX.Admin.QueryWeChatByTime1(param, function(ret) {
	AJAX.Business.QueryPaylog( JSON.stringify(param), function(ret){
	    ...
	    $('.total-info .pull-left .item:eq(1) .value').html('￥' + ret.PayAmountSum.Wechat_Amount);
