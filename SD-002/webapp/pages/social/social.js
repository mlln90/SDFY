	...
	switch (_type) {
		...
		case 1: 
			if (ALISTATE) {
				...
				$.layerPop({
					msg: '请确认社保支付功能开启？ </br> 社保支付相关功能将开启使用',
					time: 6,
					yes: function() {
						BankUpDown('true');		
					}
		...
		case 2:
			if (ALISTATE) {
				$.layerPop({
					msg: '请确认社保支付功能关闭？ </br> 社保支付相关功能将暂停使用',
					time: 6,
					yes: function() {
						BankUpDown('false');
					}
...
function QueryBankList() {
	...
	AJAX.Business.QueryPaylog( JSON.stringify(param), function(ret){
	    ...
	    $('.total-info .pull-left .item:eq(1) .value').html('￥' + ret.PayAmountSum.Social_Amount);
