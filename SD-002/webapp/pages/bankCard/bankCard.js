...
function QueryBankList() {
	...
	// AJAX.Admin.QueryPOSByTime1(param, function(ret) {
	AJAX.Business.QueryPaylog( JSON.stringify(param), function(ret){
	    ...
	    $('.total-info .pull-left .item:eq(1) .value').html('￥' + ret.PayAmountSum.Bank_Amount);
...
function TransformData(_data) {
	...
	$.each(_data, function(n, val) {
		...
		var obj = {
			...
			// Cost: val.POSJE,
			// BankCard: val.YHKH,
			// POSSerialNum: val.POSLSH,
			// POSReferNum: val.CKH,
			// BusinessType: val.BRLX,
			// HIS: val.YCJG,
			// Number: val.ZZJBH,
			// Desc: val.YHJYJGMS 
